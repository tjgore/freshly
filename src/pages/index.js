...
const Index = ({ recentBooks, popularBooks }) => (
  <Layout>
    ...
    <section className="lg:px-20 px-8 text-center mb-20">
      <h1
        id="popular"
        className="pb-6 text-4xl text-center lg:text-left font-bold text-green-400"
      >
        \\ Categories
      </h1>
...
Index.getInitialProps = async ({ req }) => {
  ...
  const [recentBooks, popularBooks] = await Promise.all([
    fetch(
      `https://api.nytimes.com/svc/books/v3/lists.json?list=Combined%20Print%20and%20E-Book%20Nonfiction&api-key=${process.env.NYT_API_KEY}`
